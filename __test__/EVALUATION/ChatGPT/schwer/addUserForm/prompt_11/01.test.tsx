...
describe('AddUserFormSchwer', () => {
    ...
    it.skip('shows error message when email is already taken', async () => {
        render(<AddUserFormSchwer setUsers={setUsers} users={users} />);
    ...
    it.skip('calls setUsers and resets form on successful submission', async () => {
        ...
        await waitFor(() => {
            expect(setUsers).toHaveBeenCalledWith([
                ...users,
                { name: 'New User', email: 'new@example.com', role: USER_ROLE.ADMIN, password: 'Password1!' },
            ]);
        });
    ...
    it.skip('shows department field when role is not CUSTOMER', async () => {
        render(<AddUserFormSchwer setUsers={setUsers} users={users} />);
